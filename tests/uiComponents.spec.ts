...
test.describe('Form Layouts Page', () => {
    ...
    test('input fields', async({page}) => {
        ...
        // to type, for example to type it slow then add delay
...
test("web tables", async({page}) => {
    ...
    const ageCell = targetRow.locator('td:has-text("30")')
    ...
    const emailCell = targetRowById.locator('td:has-text("test@test.com")')
    await expect(emailCell).toBeVisible()

    // testi filter of the table
